...
public class SSLSocketChannel2 implements ByteChannel, WrappedByteChannel {
	...
	private synchronized void processHandshake() throws IOException {
		...
		if( tasks.isEmpty() || this.sslEngine.getHandshakeStatus() == SSLEngineResult.HandshakeStatus.NEED_WRAP ) {
	...
	private synchronized ByteBuffer wrap( ByteBuffer b ) throws SSLException {
		outCrypt.compact();
        this.writeEngineResult = sslEngine.wrap( b, outCrypt );
		outCrypt.flip();
		return outCrypt;
	}
	...
	private synchronized ByteBuffer unwrap() throws SSLException {
		...
		} while ( this.readEngineResult.getStatus() == SSLEngineResult.Status.OK && ( rem != inData.remaining() || this.sslEngine.getHandshakeStatus() == HandshakeStatus.NEED_UNWRAP ) );
	...
	public boolean isNeedRead() {
		return inData.hasRemaining()
                || ( inCrypt.hasRemaining()
                        && this.readEngineResult.getStatus() != Status.BUFFER_UNDERFLOW
                        && this.readEngineResult.getStatus() != Status.CLOSED
        );
	}
