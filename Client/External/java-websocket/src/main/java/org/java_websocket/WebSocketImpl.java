...
public class WebSocketImpl implements WebSocket {
			...
			if( draft == null ) {
				...
				if( isflashedgecase == HandshakeState.MATCHED ) {
                    try {
                        ...
                        this.close(CloseFrame.ABNORMAL_CLOSE, "remote peer closed connection before flashpolicy could be transmitted", true);
                    }
					return false;
				}
			}
    ...
    @Override
    public void sendFragmentedFrame(Framedata.Opcode opcode, ByteBuffer buf, boolean end) {
        this.send(this.draft.continuousFrame(opcode, buf, end));
    }
